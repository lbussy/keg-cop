...
void setup()
{
    ...
    if (configDone != true)
        serialPrintln(F("Missing Configuration End"));
    ...
    if (useValves > 0)
        setPinsMode(numSensors, valvesPin, OUTPUT);
    for (unsigned int i = 0; i < numSensors; i++)
    {
        resetTap(i);
    ...
    for (unsigned int i = 0; i < numSensors; i++)
    {
        ...
        if (pulseCount[i] > 0)
        {
            ...
            if (reset == true)
            {
                ...
                if (pulseCount[i] > 10)
                    debug("%s %d %d", "Reset Tap during loop Pin", pulsePin[i], i);
                resetTap(i);
            }
...
void piStatusCheck()
{
    ...
    while (ii + 1 < INPUT_SIZE)
    {
        ...
        if (readMsg[ii] == 'S')
            ii = 0; //In case serial string starts over (S)tatus
        if (readMsg[ii] == '\n' || readMsg[ii] == '|' || readMsg[ii] == 0)
            break;
        ii++;
    }
    readMsg[ii] = 0;
    ...
    if (strcmp(command, "Status") == 0)
    {
        ...
        if (reconfigReq)
            reconfigRequired = atol(reconfigReq);
...
void shutDownNonPouringTaps(unsigned int currentTap)
{
    ...
    for (unsigned int i = 0; i < numSensors; i++)
    {
        if (i == currentTap)
            continue;
        if (manualValveState[i])
            continue; //User Manual Opened Valve Through UI, dont shut it off
